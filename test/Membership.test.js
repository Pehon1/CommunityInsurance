...
describe('Membership Contract', () => {
    ...
    it('Non member should not be allowed to set contract addresses', async () => {
        ...
        const user1ConnectedToContract = await membershipContract.connect(wallet1)
        await expect(user1ConnectedToContract.SetLinkedContracts(membershipContract.address)).to.be.revertedWith('Ownable: caller is not the owner')
    })
